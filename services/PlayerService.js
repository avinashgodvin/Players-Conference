...
class PlayerService {
  ...
  async getNames() {
    ...
    return data.map((player) => {
      return { name: player.name, shortname: player.shortname };
    });
  }
   ...
   * @param {*} shortname
   */
  async getPlayer(shortname) {
    const data = await this.getData();
    //console.log(data, shortname);
    const player = data.find((elm) => {
      return elm.shortname === shortname;
    });
  ...
  /***
   * To get the detailed list of top speakers
   */
  async getListOfTopPlayers() {
    const data = await this.getDataOfTopPlayers();
    return data.map((player) => {
      return {
        name: player.name,
        sport: player.sportName,
        imgpath: player.imagePath,
        shortname: player.shortname,
      };
    });
  }
  ...
  async getDataOfTopPlayers() {
    const data = await readFile(this.datafile, "utf8");
    return JSON.parse(data).topplayers;
  }
}
